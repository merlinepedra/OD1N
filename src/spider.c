...
// payload tamper, get payload of line and make tamper 
	...
	memset(pathurl,0,sizeof(char)*1023);
...
// create responses path
						memset(pathsource,0,sizeof(char)*1023);
						strlcat(pathsource,RESPONSE_PATH,33);
						strlcat(pathsource,"/",34);
						strlcat(pathsource,arg[5], 64);
...
// create datatables	
			...
			{		
			//	memset(pathsource,0,sizeof(char)*1023);
				strlcat(pathsource,RESPONSE_PATH,18);
				strlcat(pathsource,arg[5], 33);
				mkdir(pathsource,S_IRWXU|S_IRWXG|S_IRWXO);
				snprintf(pathsource,986,"/%s/%s.html",arg[5],rand_str(randname, sizeof randname));
			}
//write logs
